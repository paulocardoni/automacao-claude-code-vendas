...
import { ChevronRight, Calendar, Clock, Tag, Zap, Twitter, Instagram, Music, Search, X } from "lucide-react";
...
export default function Home() {
  ...
  function highlight(text: string, term: string): React.ReactNode {
    ...
    return parts.map((part, i) =>
      regex.test(part)
        ? <mark key={i} className="bg-[#00FF88] bg-opacity-20 text-[#00FF88] rounded px-0.5">{part}</mark>
        : part
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 bg-background border-b neon-border">
        <div className="container py-3 sm:py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-6 md:gap-8">
            ...
            <a href="https://formacaoagentica.com.br/" target="_blank" rel="noopener noreferrer" className="text-foreground hover:text-accent transition-colors text-sm">Curso</a>
            <a href="mailto:atendimento.claudetrafego@gmail.com" className="text-foreground hover:text-accent transition-colors text-sm">Contato</a>
          </nav>
        </div>
      </header>
      ...
      <section className="py-12 sm:py-16 md:py-24 bg-gradient-to-b from-card to-background px-4 sm:px-6 relative overflow-hidden">
        ...
        <div className="container max-w-4xl mx-auto text-center animate-fade-in">
          <div className="inline-block mb-3 sm:mb-4 px-3 sm:px-4 py-1.5 sm:py-2 bg-accent bg-opacity-10 border neon-border rounded-full">
            <span className="text-accent font-mono text-xs sm:text-sm font-bold">BLOG ESPECIALIZADO</span>
          </div>

          <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-bold mb-6 sm:mb-8 text-foreground leading-tight">
            Trilha de notícias e aprendizado para profissionais e agências
          </h1>

          {/* ── Campo de busca ── */}
          <div className="max-w-xl mx-auto relative">
            <div className="flex items-center gap-2 bg-[rgba(26,26,26,0.9)] border border-[rgba(0,255,136,0.3)] rounded-xl px-4 py-3 focus-within:border-[#00FF88] focus-within:shadow-[0_0_0_3px_rgba(0,255,136,0.08)] transition-all duration-200">
              <Search size={18} className="text-[#00FF88] shrink-0 opacity-70" />
              <input
                type="text"
                value={query}
                onChange={e => setQuery(e.target.value)}
                placeholder="Buscar por palavra-chave, tema ou categoria..."
            ...
            {isSearching && (
              <p className="text-xs text-muted-foreground mt-2 font-mono">
                {filteredPosts.length === 0
                  ...
                  : `${filteredPosts.length} artigo${filteredPosts.length > 1 ? "s" : ""} encontrado${filteredPosts.length > 1 ? "s" : ""} para "${query}"`
                }
              </p>
            )}
          </div>
          ...
          {/* CTA */}
          {!isSearching && (
            <div className="flex gap-3 sm:gap-4 justify-center flex-wrap mt-6 sm:mt-8">
              <a
      ...
      {/* ── Top 5 — só exibe quando NÃO está buscando ── */}
      {!isSearching && (
        <div className="border-b border-[rgba(0,255,136,0.12)]">
          <TopPosts />
        </div>
      )}
        ...
        <div className="container max-w-5xl mx-auto">
          <div className="mb-8 sm:mb-12">
            <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold mb-3 sm:mb-4">
              {isSearching ? "Resultados da busca" : "Últimos Artigos"}
            </h2>
          ...
          {isSearching && filteredPosts.length === 0 && (
            <div className="text-center py-16 sm:py-24">
              ...
              <p className="text-muted-foreground text-sm">
                ...
                <button onClick={() => setQuery("")} className="text-[#00FF88] hover:underline">
                  ...
                  <div className="grid md:grid-cols-3 gap-4 sm:gap-6 md:gap-8 items-center">
                    ...
                    <div className="md:col-span-2 order-1 md:order-2">
                      ...
                      <h3 className="text-lg sm:text-2xl md:text-3xl font-bold mb-2 sm:mb-3 group-hover:text-accent transition-colors leading-tight">
                        {isSearching ? highlight(post.title, query) : post.title}
                      </h3>

                      <p className="text-muted-foreground mb-3 sm:mb-4 line-clamp-2 text-sm sm:text-base">
                        {isSearching ? highlight(post.excerpt, query) : post.excerpt}
                      </p>
                      ...
                      <div className="flex flex-wrap gap-2 mt-3 sm:mt-4">
                        {post.tags.slice(0, 3).map(tag => (
                          <span key={tag} className="text-xs text-muted-foreground flex items-center gap-1">
                            <Tag size={12} />
                            {isSearching ? highlight(tag, query) : tag}
                          </span>
                        ))}
                      </div>
                    </div>
        ...
        <div className="container max-w-5xl mx-auto">
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 mb-6 sm:mb-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-xs sm:text-sm">
                <li><a href="/blog" className="text-muted-foreground hover:text-accent transition-colors">Blog</a></li>
                <li><a href="https://formacaoagentica.com.br/" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-accent transition-colors">Curso</a></li>
              </ul>
            </div>
            <div>
              <h4 className="font-bold mb-3 sm:mb-4 text-sm sm:text-base">Legal</h4>
              <ul className="space-y-2 text-xs sm:text-sm">
                <li><a href="#" className="text-muted-foreground hover:text-accent transition-colors">Privacidade</a></li>
                <li><a href="#" className="text-muted-foreground hover:text-accent transition-colors">Termos</a></li>
              </ul>
            </div>
            <div>
          ...
          <div className="border-t neon-border pt-6 sm:pt-8">
            <div className="flex justify-center gap-6 mb-6">
              <a href="https://x.com/IA_para_GDT" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-accent transition-colors"><Twitter size={24} /></a>
              <a href="https://www.instagram.com/claudecodegtt/" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-accent transition-colors"><Instagram size={24} /></a>
              <a href="https://www.tiktok.com/@claudecodegtt" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-accent transition-colors"><Music size={24} /></a>
            </div>
